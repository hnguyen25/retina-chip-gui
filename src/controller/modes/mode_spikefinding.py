...
def setup_spike_finding(app, CURRENT_THEME, themes, NUM_CHANNELS_PER_BUFFER):
    ...
    """
    # mode = app.chart_update_extra_params["arrayMap"]
    print("Setup Spike Finding is running.")
    defaultMode = "Summary"
    ...
    from src.controller.modes.mode_spikefinding import OnRewind, OnPlay, OnFastForward, ChangeMode
...
def ChangeMode(app):
    ...
    """
    mode = app.chart_update_extra_params['arrayMap']
    mode = 'Summary' if mode == 'Real-Time' else 'Real-Time'
    app.chart_update_extra_params['arrayMap'] = mode

    app.ModeButton.setText(mode)